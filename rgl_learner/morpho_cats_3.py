...
def learn(source, lang, filename=None,
    ...
    def sort_table(table):
        new_table = {}
        for k, v in sorted(table.items(), key = lambda x: (1,x[0]) if x[0] != "s" else (0,x[0])):
    ...
    def complete_table(table, param2val):
        ...
        pars = [params[val][-1] for val in vals if val in params and params[val]]
    ...
    for pos, ts in tables.items():
        for (word, table, gtags) in ts:
            ...
            if complete: 
                table = complete_table(table, param2val) # remake param2val
